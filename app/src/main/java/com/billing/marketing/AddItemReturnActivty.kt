...
import com.billing.marketing.databinding.ActivityAddItemReturnActivtyBinding
...
class AddItemReturnActivty : AppCompatActivity() {
    ...
    private lateinit var taxRate: String
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnSave2.setOnClickListener {
            ...
            if (taxRate <= "0" || taxRate.toInt() < qnt.toInt()){
                Toast.makeText(this,"Not enough stock available for this product.", Toast.LENGTH_SHORT).show()
            }
            else {
...
//        binding.tvGst.setOnClickListener { showGstPopupMenu() }
    ...
    private fun handleProductListResponse(result: List<ProductDetails>) {
        ...
        binding.etItemName.setOnItemClickListener { parent, _, position, _ ->
            ...
            val inStock = result.find { it.in_stock == selectedProductName }
            selectedProduct?.let {
...
//        nameAdapter.notifyDataSetChanged()
    ...
    private fun calculateTotalAmount() {
        ...
        binding.tvTotalPrice.text = Editable.Factory.getInstance().newEditable(amountPrice)
    }
}
